...
def check_qwen_scene_api():
    ...
    print("[QWEN API] llama-server가 꺼져 있으면 화면 해설 TTS는 생략합니다.")
    return True
...
def clean_qwen_output(text):
    ...
    # 작은 Qwen이 가끔 어색한 직역/외래어를 내는 경우 후처리
    replacements = {
        "맨은": "사람은",
        "맨이": "사람이",
        "맨은 ": "사람이 ",
        "맨 ": "사람 ",
        "노트북 컴퓨터 앞에 앉아 있습니다": "노트북 컴퓨터 근처에 사람이 보입니다",
        "컴퓨터 앞에 앉아 있습니다": "컴퓨터 근처에 사람이 보입니다",
        "컴퓨터가 책상 위에 앉아 있다": "책상 위에 컴퓨터가 보입니다",
        "컴퓨터가 책상 위에 앉아 있습니다": "책상 위에 컴퓨터가 보입니다",
        "노트북 컴퓨터가 책상 위에 앉아 있습니다": "책상 위에 노트북 컴퓨터가 보입니다",
    }
...
def is_valid_korean_scene_message(text):
    ...
    if english_letters > 0:
        return False
    # BLIP의 부정확한 man/person 번역이 너무 어색하게 나오면 재시도
    awkward_terms = ["맨", "남성", "남자"]
    if any(term in text for term in awkward_terms):
        return False
    return True
...
def caption_to_scene_message(caption):
    ...
    system_prompt = (
        "너는 시각장애인용 보행 보조 장치의 화면 해설 문장을 만드는 도우미다. "
        "입력은 영어 이미지 캡션이다. 반드시 한국어로만 답하라. "
        "중국어, 영어, 한자, 외래어식 직역은 사용하지 마라. "
        "캡션을 그대로 직역하지 말고, 사용자가 듣기 자연스러운 화면 해설로 바꿔라. "
        "사람의 성별이 확실하지 않으면 남자, 남성, 맨이라고 하지 말고 사람이라고 말하라. "
        "위치 관계가 애매하면 단정하지 말고 '근처에 보입니다'처럼 완만하게 말하라. "
    ...
    attempts = [
        ...
        [
            ...
            {"role": "assistant", "content": "방 안에 사람이 보입니다."},
            {"role": "user", "content": "영어 캡션: a laptop computer sitting on top of a desk\n한국어 화면 해설:"},
            {"role": "assistant", "content": "책상 위에 노트북 컴퓨터가 보입니다."},
            {"role": "user", "content": "영어 캡션: a man sitting in front of a laptop computer\n한국어 화면 해설:"},
            {"role": "assistant", "content": "노트북 컴퓨터 근처에 사람이 보입니다."},
            {"role": "user", "content": f"영어 캡션: {caption}\n한국어 화면 해설:"},
        ],
    ]
    ...
    try:
        ...
        for idx, messages in enumerate(attempts, start=1):
            ...
            print(f"[QWEN API SKIP] 한국어/자연스러움 검증 실패. 재시도 또는 생략: {message}")

        print(f"[QWEN API SKIP] 한국어 화면 해설 생성 실패. 영어 TTS 생략: caption={caption} / last={last_message}")
        return "", "skip"

    except urllib.error.URLError as e:
        print(f"[QWEN API 오류] llama-server 연결 실패. 화면 해설 TTS 생략: {e}")
        return "", "skip"
    except Exception as e:
        print(f"[QWEN API 오류] 화면 해설 변환 실패. 화면 해설 TTS 생략: {e}")
        return "", "skip"
...
def scene_speak_tts(message, lang="ko"):
    ...
    if not message or lang != "ko":
        print(f"[SCENE TTS SKIP] 한국어 화면 해설이 아니므로 생략: lang={lang}, message={message}")
        return
        ...
        try:
            ...
            if caption:
                ...
                if message and message_lang == "ko":
                    ...
                    print("[SCENE SKIP] 한국어 화면 해설이 없어 TTS/이벤트 전송 생략")
