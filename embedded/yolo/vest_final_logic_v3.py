...
def make_message_candidate(source, direction, label=None, distance=None):
    """방향, 거리(cm), 객체명을 이용해 TTS 안내 문장을 생성한다."""
    direction_kr = {
        "LEFT": "좌측",
        "CENTER": "중앙",
        "RIGHT": "우측"
...
def speak_tts(message):
    """gTTS로 한국어 안내 음성을 생성하고 기본 오디오 장치로 출력한다."""
...
print("Vest Final Logic V3 시작")
print("정책:")
print("- 1순위: 초음파 50cm 이하 STRONG, 50~100cm SHORT + TTS 거리 안내")
print("- 2순위: 카메라 CENTER 위험 객체만 보수적으로 진동 + TTS 객체 안내")
print("- 사람(person)은 진동 없이 TTS 안내만 수행")
print("- MiDaS는 상대 깊이 기반이므로 실제 거리 m로 직접 사용하지 않음")
print("- unknown/tree는 표시 및 메시지 후보만")
print("종료: q")
...
try:
    while True:
        ...
        if risk_result is not None:
            # 카메라만으로는 실제 m 거리 산출이 어렵기 때문에,
            # 초음파 방향과 카메라 방향이 일치할 때만 초음파 거리값을 같이 사용한다.
        ...
        cv2.imshow("Vest Final Logic V3", frame)
