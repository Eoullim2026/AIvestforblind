...
SCENE_TTS_SUPPRESS_AFTER_ALERT = 4.0
...
def caption_to_korean_scene(caption):
    """
    BLIP 영어 캡션을 시연용 한국어 화면 해설로 가볍게 변환한다.
    Qwen 호출은 업로드 파일에 포함되어 있지 않아, 여기서는 안전한 규칙 기반 문장으로 정리한다.
    """
    if not caption:
        return "현재 화면 정보를 해설하기 어렵습니다."
    ...
    objects = []
    for keyword, korean in object_map:
        if keyword in c and korean not in objects:
            objects.append(korean)

    place = None
    for keyword, korean in place_map:
        if keyword in c:
            place = korean
            break

    if objects and place:
        return f"현재 {place}으로 보이며, {', '.join(objects[:3])}이 보입니다."

    if objects:
        return f"현재 화면에는 {', '.join(objects[:3])}이 보입니다."

    return "현재 화면에 주변 사물이 보입니다."


def scene_speak_tts(message):
    ...
    if now - last_tts_time < SCENE_TTS_SUPPRESS_AFTER_ALERT:
        debug_log("[SCENE] 최근 경고 TTS 때문에 화면 해설 TTS 생략")
        return

    try:
        print(f"[SCENE TTS] {message}")
        tts = gTTS(text=message, lang="ko")
...
def make_scene_event_payload(caption, message):
    return {
        "name": "화면 해설",
        "direction": "중앙",
        "distance": "-",
...
def scene_caption_worker(event_sender):
    ...
    while not scene_caption_stop:
        ...
        try:
            ...
            if caption:
                message = caption_to_korean_scene(caption)
                ...
                if SCENE_CAPTION_EVENT_ENABLE and event_sender is not None:
                    event_sender.send_event(make_scene_event_payload(caption, message))

                put_latest(scene_tts_queue, message)
...
def scene_tts_worker():
    ...
    while not scene_caption_stop:
        try:
            message = scene_tts_queue.get(timeout=0.5)
        except queue.Empty:
            continue
...
def submit_scene_caption_if_needed(frame, ultra_detected=False, risk_result=None):
    ...
    if ultra_detected:
        debug_log("[SCENE] 초음파 위험 감지 중이라 화면 해설 생략")
        return

    if risk_result is not None and risk_result.get("risk") == "HIGH":
        debug_log("[SCENE] 카메라 HIGH 위험 감지 중이라 화면 해설 생략")
        return

    if scene_caption_running:
        debug_log("[SCENE] 이전 화면 해설 처리 중이라 이번 프레임 생략")
        return

    put_latest(scene_caption_queue, frame.copy())
    debug_log("[SCENE] 화면 해설 프레임 제출")
