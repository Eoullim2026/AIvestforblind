...
def find_near_obstacle_from_depth(depth_map, frame_width, frame_height):
    """
    MiDaS depth map에서 '장애물다운 가까운 영역'만 추출한다.
    너무 넓은 벽/바닥/책상/전체 면은 제외한다.
    """

    valid_depth = depth_map.copy()

    # 화면 하단 바닥/자기 몸/노이즈 영역 제외
    ...
    while True:
        ...
        if risk_result:
            print(
                ...
                f"confirm: {near_confirm_count}"
            )

            current_direction = risk_result["direction"]

            if last_near_direction == current_direction:
            ...
            if risk_result["matched"] and risk_result["risk"] == "HIGH":
                if near_confirm_count >= 1:
                    trigger_vibration(risk_result["direction"], pattern="DOUBLE")

            elif risk_result["matched"] and risk_result["risk"] == "MEDIUM":
                if near_confirm_count >= 2:
                    trigger_vibration(risk_result["direction"], pattern="SHORT")

            else:
                if near_confirm_count >= DEPTH_CONFIRM_FRAMES:
                    trigger_vibration(risk_result["direction"], pattern="SHORT")
        ...
        cv2.imshow("MiDaS First Vest V2", frame)
