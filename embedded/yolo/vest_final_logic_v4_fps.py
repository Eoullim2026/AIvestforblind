...
# FPS 개선 설정
YOLO_IMGSZ = 320
MIDAS_INTERVAL = 3          # 기존 1 -> 3: MiDaS를 3프레임마다 실행
...
def trigger_vibration(direction, pattern="SHORT", source="CAMERA"):
    """
    기존 코드는 이 함수 내부의 time.sleep 때문에 메인 루프가 멈췄음.
    개선 버전은 진동 패턴을 별도 스레드에서 실행한다.
    """
...
def update_ultrasonic_if_needed(frame_count):
    """
    초음파를 매 프레임 측정하지 않고 ULTRA_INTERVAL마다 측정한다.
    측정하지 않는 프레임에서는 직전 값을 재사용한다.
    """
    ...
    if frame_count % ULTRA_INTERVAL == 0:
        ...
        # 기존 0.02초는 FPS 저하가 커서 0.005초로 축소
        time.sleep(0.005)
...
print("Vest Final Logic V4 FPS 시작")
print("정책:")
...
try:
    while True:
        ...
        cv2.imshow("Vest Final Logic V4 FPS", frame)
