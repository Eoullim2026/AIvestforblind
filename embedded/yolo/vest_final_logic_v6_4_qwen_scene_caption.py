...
# Qwen 화면 해설 변환 설정
# =====================================================
# BLIP 영어 캡션을 Qwen으로 한국어 보행 안내문으로 바꾼다.
# 모델 경로는 Jetson에 이미 받아둔 로컬 경로가 있으면 그 경로로 바꾸는 것을 권장한다.
...
def load_qwen_scene_model():
    """BLIP 영어 캡션을 한국어 안내문으로 바꿀 Qwen 모델을 로드한다."""
    if not QWEN_SCENE_ENABLE:
        return None, None, None, False

    q_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    try:
        print("Qwen 화면 해설 변환 모델 로딩 중...")
        ...
        print(f"Qwen 화면 해설 변환 모델 로드 완료: {q_device}")
        return tokenizer, model, q_device, True

    except Exception as e:
        print(f"[QWEN] 모델 로드 실패. 화면 해설은 영어 캡션 그대로 사용합니다: {e}")
        return None, None, None, False


qwen_tokenizer, qwen_model, qwen_device, qwen_available = load_qwen_scene_model()
...
def caption_to_scene_message(caption):
    """BLIP 영어 캡션을 Qwen으로 한국어 보행 안내문으로 변환한다."""
    caption = clean_blip_caption(caption)

    if not caption:
        return "현재 화면을 설명하기 어렵습니다.", "ko"

    if not qwen_available or qwen_tokenizer is None or qwen_model is None:
        # Qwen 로드 실패 시 영어 원문으로 fallback
        return caption, "en"

    system_prompt = (
        "너는 시각장애인 보행 보조 장치의 화면 해설 문장을 만드는 도우미다. "
        "영어 이미지 캡션을 한국어 한 문장으로 자연스럽게 바꿔라. "
        "원문에 없는 물체, 위험, 거리, 방향은 절대 추가하지 마라. "
        "위험 판단을 새로 만들지 말고 보이는 장면만 짧게 말해라. "
        "출력은 한국어 한 문장만 한다."
    )
    user_prompt = f"영어 화면 설명: {caption}"
    ...
    try:
        ...
        message = clean_qwen_output(response)

        if not message:
            return caption, "en"

        if QWEN_PRINT_DEBUG:
            print(f"[QWEN SCENE] {message}")

        return message, "ko"

    except Exception as e:
        print(f"[QWEN 오류] 화면 해설 변환 실패. 영어 캡션 그대로 사용: {e}")
        return caption, "en"
...
print("Vest Final Logic V6.4 Qwen Scene Caption Debug 시작")
print("정책:")
...
try:
    while not shutdown_requested:
        ...
        # 10. 로컬 화면 표시
        # -----------------------------
        cv2.imshow("Vest Final Logic V6.4 Qwen Scene Caption", frame)
